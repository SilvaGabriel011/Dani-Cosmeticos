...
export async function POST(
  ...
  try {
    ...
    if (sale.status === "CANCELLED") {
      return NextResponse.json(
        { error: { code: "SALE_CANCELLED", message: "Não é possível adicionar pagamento a uma venda cancelada" } },
        { status: 400 }
      )
    }
    ...
    if (amount > remainingAmount + 0.01) {
      return NextResponse.json(
        { error: { code: "AMOUNT_EXCEEDS", message: `Valor máximo permitido: R$ ${remainingAmount.toFixed(2)}` } },
        { status: 400 }
      )
    }
