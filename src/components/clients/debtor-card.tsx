...
import { Debtor } from "@/hooks/use-debtors"
      ...
      <CardContent className="p-4">
        ...
        <div className="mt-4 flex items-center justify-between">
          <span className="text-sm text-muted-foreground">
            {debtor.salesCount} {debtor.salesCount === 1 ? "compra" : "compras"} pendente{debtor.salesCount !== 1 ? "s" : ""}
          </span>
          <Button
            variant="ghost"
            size="sm"
        ...
        {isExpanded && (
          <div className="mt-4 space-y-4 border-t pt-4">
            {debtor.sales.map((sale) => {
              const totalPaid = sale.receivables.reduce(
                (sum, r) => sum + Number(r.paidAmount),
                0
              )
              const totalAmount = sale.receivables.reduce(
                (sum, r) => sum + Number(r.amount),
                0
              )
              const paidInstallments = sale.receivables.filter(
                (r) => r.status === "PAID"
              ).length
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">
                        Compra {formatDate(sale.createdAt)}
                      </span>
                      <span className="text-sm text-muted-foreground">
